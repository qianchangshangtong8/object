...
margin:0 auto;
  ...
  if(currentusername.equals(username)&&!("".equals(currentusername))){
   %>
  <div id="clear"><a href="upload_image.jsp?username=<%=currentusername %>&&numberpage=3" class="button">上传图片</a></div>
  <%
    }
   %>
...
<p>已上传的 <select name="type" id="selecttype" onchange="loadType();">
   ...
   <div class="showImage" onmouseover="showDel(<%=image.getId() %>)" onmouseout="hideDel(<%=image.getId() %>)">  
	<div class="thumbnail">
		<a href="<%=image.getImage_path() %>" rel="lightbox" title="">
		  <img src="<%=image.getImage_path() %>" width="200" height="200" alt="图片说明:<%=image.getMessage() %>"/>
		</a>
